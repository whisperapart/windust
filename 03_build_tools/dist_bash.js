/**
 * Peerless build tool, for mac-osx
 * @Author: Jim Dai
 * @Date: 2023.07.20
 * @platform: mac-osx
...
function bundleCSS(){
    console.log("=== 压缩CSS文件 ===");
    // let assetsDirCmd = 'mkdir -p '+FOLDER_MIN_PATH+'www/assets/css';
    // execCmd(assetsDirCmd);
...
function mini_json_file_or_copy_other_files_in_folder(source_dir,dest_dir){
    ...
    fs.readdir(source_dir, function (err, files) {
        ...
        files.forEach(function (file) {
            ...
            if(fileExt === '.json'){
                miniJsonFile(source_dir+file, dest_dir+file);
            }else{
                console.log("复制文件：" + source_dir+file + ' '+dest_dir+file);
                execCmd('cp ' + source_dir+file + ' '+dest_dir+file);
            }
        });
    });
     **/
    ...
    for(let i=0;i<files.length;i++){
        ...
        if(fileExt === '.json'){
            ...
            execCmd('cp ' + source_dir+files[i] + ' '+dest_dir+files[i]);
        }
    }
}
...
function bundleFolders(){
    ...
    for(let i=0;i<folders.length;i++){
        console.log('复制'+'../01_src/www/'+folders[i]+ ' '+ ''+FOLDER_MIN_PATH+'www/'+folders[i]);
        execCmd('cp -P -R '+'../01_src/www/'+folders[i]+ ' '+ ''+FOLDER_MIN_PATH+'www/'+folders[i]);
    }
}
...
function main() {
    ...
    if((platform !== 'ios') && (platform !== 'windows') && (platform !== 'osx') && (platform !== 'browser')){
        console.log("platform error, supported = ios | windows | osx | browser");
        return;
    }
    ...
    execCmd("cp -R "+FOLDER_MIN_PATH + '/www/ ' + CORDOVA_PRJ_PATH+'www/');
    // 02_plat_spec files to 08_dis
    console.log("==> copy 02_plat_spec files to 08_dist");
    execCmd("cp "+PLAT_SPEC_PATH + actPlat + '/www/extra.js '+ CORDOVA_PRJ_PATH + 'www/js/extra.js');
    execCmd("cp "+PLAT_SPEC_PATH  + 'config.xml '+ CORDOVA_PRJ_PATH + 'config.xml');
    execCmd("cp "+PLAT_SPEC_PATH  + 'package.json '+ CORDOVA_PRJ_PATH + 'package.json');
    execCmd("cp -R "+PLAT_SPEC_PATH + actPlat + '/cordova/ '+ dist_path + 'cordova/');
    execCmd("cp -R "+PLAT_SPEC_PATH + actPlat + '/platform_www/ '+ dist_path + 'platform_www/');
    ...
    execCmd("mv -f "+ dist_path+'build ' + release_path);
    execCmd("open "+release_path);
}
